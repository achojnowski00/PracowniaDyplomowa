...
export const LeftPanel = () => {
    ...
    const handleCreateBudget = async () => {
        ...
        await axios
            .post("http://localhost:8000/api/budgets/create", {
                name: budgetName,
            }, {
                method: "POST",
                headers: {
    ...
    return (
        <div className="leftPanel">
            {!wantChangeName && <h1 onClick={handleNameClick} className="leftPanel__title">Cześć {userdata.name}!</h1>}

            {wantChangeName && (
                <h1 className="leftPanel__title">Cześć 
            ...
            {userdata.name === "Twoja nazwa użytkownika" && !wantChangeName && <h6 className="leftPanel__info">Kliknij nazę użytkownika aby ją zmienić</h6>}


            
            {!userdata && <div className="leftPanel__loading"><CircularProgress /></div>}
            {userdata.budgets && (
                <ul 
                ...
                {userdata.budgets.map((budget) => {
                    return (
                        <li 
                            key={budget.id}
                            className={currentBudget === budget.id ? "leftPanel__list-item leftPanel__list-item--active" : "leftPanel__list-item"}
                            onClick={() => {
            ...
            <button className="leftPanel__logoutBtn" onClick={handlelogout}>Wyloguj</button>
        </div>
    );
};
